...
def get_angle(X, tg_resi, tm, tm_a_j):
	...
	cmd.iterate('model ' + 'tm' + str(tm) + '_a and name N', 'a_resis.add(resi)')
	a_begin = int(sorted(list(a_resis))[0])
	a_end = int(sorted(list(a_resis))[-1])
	a_resis.clear()

	cmd.iterate('model ' + 'tm' + str(tm) + '_i and name N', 'i_resis.add(resi)')
	...
	tm_a_j = 0
	try:
		angle_b = cmd.get_angle('model ' + 'tm' + str(tm) + '_i and resi ' + str(i_begin + 2) + ' and name CA and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_i and resi ' + str(tg_resi) + ' and name CA and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_a and resi ' + str(a_begin + 2 + tm_a_j) + ' and name CA and alt A+\"\"')
	except:
		angle_b = cmd.get_angle('model ' + 'tm' + str(tm) + '_i and resi ' + str(i_begin + 2) + ' and name N and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_i and resi ' + str(tg_resi) + ' and name N and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_a and resi ' + str(a_begin + 2 + tm_a_j) + ' and name N and alt A+\"\"')
	try:
		angle_a = cmd.get_angle('model ' + 'tm' + str(tm) + '_i and resi ' + str(i_end - 2) + ' and name CA and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_i and resi ' + str(tg_resi) + ' and name CA and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_a and resi ' + str(a_end - 2 + tm_a_j) + ' and name CA and alt A+\"\"')
	except:
		angle_a = cmd.get_angle('model ' + 'tm' + str(tm) + '_i and resi ' + str(i_end - 2) + ' and name N and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_i and resi ' + str(tg_resi) + ' and name N and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_a and resi ' + str(a_end - 2 + tm_a_j) + ' and name N and alt A+\"\"')
...
def exp_smooth(Y, exp_step):
	...
	for idx, y in enumerate(Y):
		if idx == 0:
			...
			s = sum_rms[idx - 1][1] + float(exp_step) * (Y[idx] - sum_rms[idx - 1][1])
			sum_rms.append([idx, s])
	return sum_rms
...
def get_min_resi(min_rms, X, Y):
	idx_base = min_rms[0]
	y_min = None
	if (idx_base + SMOOTH_STEP - 1) < len(Y):
		y_min = min(Y[idx_base:idx_base + SMOOTH_STEP])
	else:
...
def get_rmsd(obj_1, obj_2, mmaker):
	rmsd = cmd.rms_cur('model ' + obj_1, 'model ' + obj_2, matchmaker = mmaker)
	return rmsd
...
def get_transforms(n_resi, tm_i_begin, tm_i_end, tm):
	...
	cmd.align('tm_i_transformed', 'tm' + str(tm) + '_a', cycles=0, transform=0, object='aln')
	rmsd = get_rmsd('tm_i_transformed & aln', 'tm' + str(tm) + '_a & aln', -1)
...
def get_best_transforms(tm_i_begin, tm_i_end, tm_a_begin, tm_a_end, tm):
	...
	for j in range(tm_i_begin, tm_i_end + 1):
		...
		cmd.iterate('model ' + 'tm' + str(tm) + '_a and resi ' + str(tm_a_j) + ' and name N', 'resns_a.add(resn)')
		cmd.iterate('model ' + 'tm' + str(tm) + '_i and resi ' + str(j) + ' and name N', 'resns_i.add(resn)')
		...
		if rms_cur == -1:
			try:
				...
				rms_cur = cmd.get_distance('model ' + 'tm' + str(tm) + '_i and resi ' + str(j) + ' and name N and alt A+\"\"', 'model ' + 'tm' + str(tm) + '_a and resi ' + str(tm_a_j) + ' and name N and alt A+\"\"')
        ...
        X, Y = clear_rms_array(X, Y)

	if EXP:
		smoothed_data = exp_smooth(Y, EXP_STEP)
	else:
        	smoothed_data = smooth_data(Y, SMOOTH_STEP)


	sm_X = [x[0] for x in smoothed_data]
	sm_Y = [x[1] for x in smoothed_data]

	_, _, x_minm, y_minm = get_extremums(sm_X, sm_Y)
	...
	if SLOPE:
		...
		y = min(y_minm)
		x = [t[0] for t in zip(x_minm, y_minm) if t[1] == y][0] + X[0]
...
for idx in data.index:
	...
	for tm in [4]:
		...
		try:
			...
			cmd.align('tm' + str(tm) + '_i', 'tm' + str(tm) + '_a', cycles=0, transform=0, object='aln')
			rmsd_i_a = get_rmsd('tm' + str(tm) + '_i & aln', 'tm' + str(tm) + '_a & aln', -1)
			...
			if pro_resi != 0 and pro_transform_b != 'PRO is in the end':
				...
				if int(pro_resi) > 1000:
					pro_point_resi = pro_resi % 1000
